var pedigreejs = (function (exports) {
	...
	**/
	...
	function img_download(opts, resolution, img_type) {
	  let fileName = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : "plot.png";
	  return new Promise((resolve, reject) => {
	    // Add reject to handle errors
	...
	function img_downloadOld(opts, resolution, img_type) {
	...
	function save_file2(opts, content, filename, type, userId) {
	  return new Promise((resolve, reject) => {
	    if (!filename) {
	...
	async function save$1(opts) {
	  try {
	    const jsonContent = JSON.stringify(current(opts));
	    ...
	    if ($("#" + opts.targetDiv).find("svg").length > 0) {
	      // Check if SVG exists
	      try {
	    ...
	    const response = await fetch(`${parentOrigin}/db/insert_family.php`, {
	      ...
	      body: JSON.stringify({
	        csv: savedJsonPath ? {
	          // only include if successfully generated
	          filename: jsonFileName,
	          // Use jsonFileName here
	          content: jsonContent
	        } : null,
	...
	var io = /*#__PURE__*/Object.freeze({
		...
		img_downloadOld: img_downloadOld,
		load_data: load_data,
		print: print,
