 ...
 */
export function img_download(opts, resolution, img_type,fileName = "plot.png") {
  return new Promise((resolve, reject) => { // Add reject to handle errors
    ...
    $.when.apply($, [deferred]).done(function () {
      ...
      // Use a more robust method for creating download links
      ...
      resolve("plot.png"); // Resolve with the filename

    }).fail(reject); // Reject if svg2img fails
  });
}
export function img_downloadOld(opts, resolution, img_type) {
...
// Function to save files (adapt if not using File System Access API)
export function save_file2(opts, content, filename, type, userId) {
  return new Promise((resolve, reject) => {
    if (!filename) {
      filename = userId ? `ped_${userId}.json` : "ped.json";
    }
...
async function save$1(opts) {
  try {
    const jsonContent = JSON.stringify(pedcache.current(opts));
    const userId = opts.userId;
    const jsonFileName = userId ? `ped_${userId}.json` : "ped.json"; // Use json extension

    const savedJsonPath = await save_file2(opts, jsonContent, jsonFileName, "application/json", userId);


    let imgContentBase64 = null;
    const imgFileName = userId ? `plot_${userId}.png` : "plot.png"; // Use plot and png extension


    if ($("#" + opts.targetDiv).find("svg").length > 0) { // Check if SVG exists
      try {
        const imgBlobPromise = img_download(opts, 1, "image/png",imgFileName); // Use img_download for PNG
        const savedImagePath = await imgBlobPromise; // Wait for the download to complete

          if (savedImagePath) { 
            const blob = await fetch(savedImagePath).then(r => r.blob());
            imgContentBase64 = await blobToBase64(blob);
          }

      } catch (imgError) {
        console.error("Image download/save error:", imgError);
      }
    } else {
    ...
    try {
      ...
      parentOrigin = window.location.protocol + "//" + window.location.hostname;
    }

    const response = await fetch(`${parentOrigin}/db/insert_family.php`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        csv: savedJsonPath ? { // only include if successfully generated
          filename: jsonFileName,  // Use jsonFileName here
          content: jsonContent,
        } : null,
        img: imgContentBase64 ? {
...
console.log('Raw response:', rawText); // See what you're actually getting
...
try {
  ...
  console.error('JSON parse error:', e);
    ...
    // Navigate to the parent page after successful save
    window.parent.location.href = `${parentOrigin}/Patient_Eligibility_for_NCCN_Test.php`;

    //const parentOrigin = window.parent.location.origin; // May be blocked by CORS depending on context
//window.parent.location.href = `${parentOrigin}/Lama2/lama/Patient_Eligibility_for_NCCN_Test.php`;

  } catch (error) {
    console.error("Error during save process:", error);
    // ... appropriate error handling (e.g., display an error message to the user)
  }
}
